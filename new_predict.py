...
def load_data(filename, seq_len, normalise_window):
    ...
    for index in range(len(yy_data) - sequence_length + 1):
        t = yy_data[index+seq_len]/yy_data[index+seq_len-1]-1
        y_result.append(t)
...
def build_model(layers, input_nums):
    ...
    model.add(Dense(layers[3],W_regularizer=regularizers.l2(0.01), activation='sigmoid'))
    
    start = time.time()
    rmsprop = optimizers.RMSprop(lr=0.02)
    #model.compile(loss="categorical_crossentropy", optimizer=rmsprop,metrics=['categorical_accuracy'])
    model.compile(loss="MAPE", optimizer=rmsprop,metrics=['MAPE', 'MSE', 'MAE'])
...
def cal_metrics(predict, y_test):
    y_pred = np.array([da[0] for da in predict])
    ...
    count = np.where(correct>0)[0].shape
    print('accuracy:', count[0] * 1.0 / correct.shape[0])
...
if __name__=='__main__':
    ...
    model3 = build_model([1, 3*seq_len, 3*seq_len, 1], len(x_train))
    ...
    print(point_by_point_predictions_3, y_train)
    cal_metrics(point_by_point_predictions_3, y_train)
    print('testing set...')
    ...
    cal_metrics(point_by_point_predictions_3, y_test)
    print('Training duration (s) : ', time.time() - global_start_time)
